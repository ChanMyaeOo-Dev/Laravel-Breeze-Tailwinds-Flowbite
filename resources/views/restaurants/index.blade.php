            ...
            <tbody>
                @foreach ($restaurants as $restaurant)
                    <tr class="bg-neutral-brand-soft border-b border-default hover:bg-neutral-secondary-medium">
                        ...
                        <td class="px-6 py-4">
                            {{ \Carbon\Carbon::parse($restaurant->opening_time)->format('H:i') }} -
                            {{ \Carbon\Carbon::parse($restaurant->closing_time)->format('H:i') }}
                        </td>
                        <td class="px-6 py-4">
                            ...
                            @else
                                <span
                                    class="bg-red-100 text-red-800 text-xs font-medium me-2 px-2.5 py-0.5 rounded dark:bg-red-900 dark:text-red-300">Inactive</span>
                            @endif
                        </td>
                        <td class="px-6 py-4 text-right">
                            ...
                            <button type="button" x-data=""
                                x-on:click.prevent="$dispatch('open-modal', 'confirm-delete-{{ $restaurant->id }}')"
                                class="font-medium text-red-600 hover:underline">Delete</button>

                            @push('modals')
                                <x-confirm-dialog name="confirm-delete-{{ $restaurant->id }}" title="Delete Restaurant"
                                    :action="route('restaurants.destroy', $restaurant)" method="DELETE">
